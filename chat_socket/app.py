...
app = Flask(__name__)
app.config['SECRET_KEY'] = "asdasdqweqweasdasdqweqwe"
...
def handle_chat(message):
    print('Chat for session % received: %s' % (request.sid, message))
    try:
        ...
        if evt == 'join':
            ...
            if room is None:
                ...
                if user is None:
                    ...
                    if elm is not None:
                        print('Sending chat message to user %s at socket: %s' % (user, elm['sid']))
                        socketio.emit('chat', msg2send, room=elm['sid'], skip_sid=request.sid)
                    else:
                        print('User requested %s, not found...' % user)
        elif evt == 'image':
            me = get_user_by_sid(request.sid)
            msg2send = {
                'type': 'image',
